...
export default function CloudPackageSync({ 
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      toast.success("Dane wysłane do paczki w chmurze");
    } catch (e) {
      console.error(e);
      toast.error('Błąd zapisu (zbyt dużo danych?).');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      toast.success("Pobrano paczkę chmurową. Przeładowuję...");
      setTimeout(() => {
        window.location.reload();
      }, 1500);
    } catch (e) {
      console.error(e);
      toast.error("Błąd podczas pobierania paczki");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="flex flex-col gap-2 p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 glass-target relative overflow-hidden">
      ...
      <div className="flex items-center justify-between mb-2">
         <div className="flex items-center gap-3">
           ...
           <span className="text-xs font-bold dark:text-white">Paczka Synchr. (Chmura)</span>
         </div>
         {lastSync && (
