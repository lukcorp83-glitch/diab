...
export const SPORTS = [
  {
    ...
    effect: 'Często podnosi poziom cukru',
    description: 'Krótki, intensywny wysiłek uwalnia adrenalinę, która stymuluje wątrobę do wyrzutu glukozy. Często powoduje niespodziewane wzrosty glikemii w trakcie i po treningu.',
    tips: [
      'Możesz potrzebować małego bolusa przed siłownią.',
      'Uważaj na opóźnione spadki cukru (nawet do 24h po).',
      'Nie zaczynaj treningu z wysokim cukrem (powyżej 250 mg/dL).'
    ]
  },
  {
    id: 'run',
    ...
    description: 'Wysiłek tlenowy (aerobowy) gwałtownie spala glukozę, zwiększając wrażliwość na insulinę. Cukier może spadać w trakcie całego treningu.',
    tips: [
      'Zmniejsz dawkę insuliny przed biegiem.',
      'Miej przy sobie szybko przyswajalne węglowodany (soki, żele).',
  ...
  },
  {
    ...
    description: 'Ciągły wysiłek aerobowy obniża poziom glukozy, choć zazwyczaj nieco wolniej niż intensywne bieganie. Oczywiście zależy od intensywności (np. interwały mogą działać jak siłownia).',
    tips: [
      'Jedz małe porcje węglowodanów w trakcie jazdy (węglowodany złożone).',
      'Zmniejsz bazę o 30-50% min. 1,5h przed wyjazdem.',
    ]
  },
  {
    id: 'swim',
    ...
    effect: 'Szybki spadek cukru',
    description: 'Jeden z trudniejszych do kontroli sportów. Zimna woda, opór i angażowanie wszystkich partii ciała powodują szybkie spalanie energii.',
    tips: [
      'Sprawdzaj cukier przed wejściem i zaraz po wejściu z wody.',
      'Bezpieczny poziom do rozpoczęcia to często pow. 150 mg/dL.',
      'Sensory i pompy mogą tracić zasięg lub odklejać się pod wodą.'
    ]
  },
  {
    id: 'mountain',
    name: 'Spacer po górach',
    icon: Mountain,
    color: 'text-amber-500',
    bg: 'bg-amber-500/10',
    border: 'border-amber-500/20',
    effect: 'Długotrwały spadek',
    description: 'Wysiłek o niskiej do średniej intensywności rozłożony na wiele godzin. Prowadzi do wyczerpania zapasów glikogenu wegetatywnego i nocnych spadków cukru.',
    tips: [
      'Zmniejsz bazę na czas wędrówki (niektórzy zmniejszają o >50%).',
      'Podjadaj regularnie węglowodany złożone (np. batony owsiane).',
      'Zabezpiecz insulinę przed skrajnymi temperaturami.'
    ]
  },
  {
    id: 'football',
    ...
    description: 'Wysiłek interwałowy. Sprinty mogą powodować wyrzuty adrenaliny (wzrosty), ale ogólny bilans często prowadzi do spadku cukru po meczu.',
    tips: [
      'Kontroluj cukier w przerwie meczu.',
      'Miej żel pod ręką przy linii bocznej.',
      'Uważaj na hipoglikemię po zakończonym wysiłku.'
    ]
  },
  {
    id: 'basketball',
    ...
    effect: 'Wysoka intensywność',
    description: 'Dynamiczny sport z dużą ilością skoków i sprintów. Bardzo szybko zużywa zapasy energii.',
    tips: [
      'Zredukuj bolus do posiłku przed meczem.',
  ...
  },
  {
    ...
    effect: 'Długotrwały wysiłek',
    description: 'Mecz może trwać od godziny do trzech. Wymaga stałego dopływu energii i skupienia.',
    tips: [
      'Podjadaj małe porcje w trakcie przerw między gemami.',
      'Emocje (stres meczowy) mogą podnosić cukier.'
    ]
  },
  {
    id: 'yoga',
    ...
    description: 'Wysiłek statyczny, skupiony na oddechu i rozciąganiu. Zazwyczaj działa stabilizująco na poziom cukru.',
    tips: [
      'Idealny sport przy lekkich wahaniach glikemii.',
      'Pamiętaj, że niektóre pozycje (np. odwrócone) wymagają stabilnego cukru.'
    ]
  },
  {
    id: 'walk',
    ...
    description: 'Najbezpieczniejsza forma ruchu. Pomaga obniżyć cukier po posiłku bez ryzyka gwałtownej hipoglikemii.',
    tips: [
      'Świetny sposób na zbicie "wysokiego" po kolacji.',
      'Wystarczy 15-20 minut, by zauważyć różnicę.'
    ]
  },
  {
    id: 'dance',
    ...
    description: 'Pół godziny intensywnego tańca to duży wysiłek energetyczny.',
    tips: [
      'Baw się dobrze, ale zerkaj na sensor!',
      'Alkohol przy tańcu (np. na imprezie) drastycznie zwiększa ryzyko hipo.'
    ]
  }
];
  ...
  return (
    <div className="mt-4 pt-4 border-t border-slate-100 dark:border-white/5">
      <div className="flex items-center justify-between mb-2">
        <span className="text-[9px] font-black uppercase tracking-widest text-slate-400">Przebieg Glikemii</span>
...
export default function GlikoTraining({ isOpen, onClose, isGlassmorphic, user, settings, currentSugar }: GlikoTrainingProps) {
  ...
  const getTrainingAdvice = () => {
    ...
    if (currentSugar < 100) {
      return {
        text: "Cukier jest niski. Zjedz 15-20g węglowodanów prostych przed startem!",
        type: 'warning'
      };
    }

    if (isAerobic) {
      if (currentSugar > 250) return { text: "Wysoki cukier. Sprawdź ketony przed intensywnym cardio!", type: 'danger' };
      if (currentSugar < 130) return { text: "Dobry moment na start, ale miej pod ręką żel lub sok.", type: 'info' };
    } else {
      // Strength/Gym
      if (currentSugar > 230) return { text: "Wysoka glikemia. Trening siłowy może ją chwilowo podnieść przez adrenalinę.", type: 'warning' };
      if (currentSugar >= 100 && currentSugar <= 180) return { text: "Idealny poziom na trening siłowy.", type: 'success' };
    }

    return { text: "Glikemia stabilna. Pamiętaj o monitorowaniu w trakcie wysiłku.", type: 'info' };
  };
  ...
  const handleStartTraining = async () => {
    ...
    await addDoc(logsRef, {
      ...
      timestamp: Date.now(),
      notes: `Start treningu: ${sportName} (Intensywność: ${intensity === 'low' ? 'Lekka' : intensity === 'medium' ? 'Średnia' : 'Wysoka'}). Cukier: ${currentSugar || '---'} mg/dL`,
      sportId: selectedSportLog,
      intensity: intensity
    });
      ...
      <div className="flex items-center justify-between mb-2 mt-2 px-2">
        <div>
          <h2 className="text-xl font-black text-slate-800 dark:text-white flex items-center gap-2">
             <Activity className="text-emerald-500" size={24} /> GlikoTrening
          </h2>
          <p className="text-sm font-medium text-slate-500">Zarządzaj aktywnością</p>
        </div>
        {onClose && (
          <button
            onClick={() => {
            ...
            >
              {activeTraining ? (
                <div className="glass p-6 rounded-[2.5rem] border border-emerald-500/30 bg-emerald-500/5 space-y-6 text-center">
                  ...
                  <div>
                    <h3 className="text-xl font-black text-slate-800 dark:text-white uppercase tracking-tight">Trwa Trening</h3>
                    <p className="text-sm font-bold text-emerald-600 dark:text-emerald-400">
                      {SPORTS.find(s => s.id === activeTraining.sportId)?.name || 'Aktywność'}
                    </p>
                  </div>
                  <div className="flex justify-center items-center gap-6 text-slate-500 dark:text-slate-400">
                     <div className="flex flex-col items-center">
                        <span className="text-[10px] font-black uppercase tracking-widest opacity-60">Czas</span>
                        <span className="text-lg font-black text-slate-700 dark:text-slate-200">{activeTraining.duration}m</span>
                     </div>
                     <div className="w-px h-8 bg-slate-200 dark:bg-white/10" />
                     <div className="flex flex-col items-center">
                        <span className="text-[10px] font-black uppercase tracking-widest opacity-60">Intensywność</span>
                        <span className="text-lg font-black text-slate-700 dark:text-slate-200 capitalize">
                          {activeTraining.intensity === 'low' ? 'Lekka' : activeTraining.intensity === 'medium' ? 'Średnia' : 'Wysoka'}
                        </span>
                     </div>
                  </div>
                  <button
                ...
                <>
                  <div className="glass p-5 rounded-[2.5rem] border border-white/20 dark:border-white/5 space-y-4">
                    <h3 className="text-sm font-black text-slate-800 dark:text-white uppercase tracking-tight">Wybierz dyscyplinę</h3>
                  ...
                  <div className="glass p-5 rounded-[2.5rem] border border-white/20 dark:border-white/5 space-y-4">
                     <h3 className="text-sm font-black text-slate-800 dark:text-white uppercase tracking-tight flex items-center gap-2">
                        <Clock size={16} className="text-slate-400" /> Czas trwania
                     </h3>
                     ...
                     <div className="pt-2">
                       <h3 className="text-sm font-black text-slate-800 dark:text-white uppercase tracking-tight mb-3">Intensywność</h3>
                  ...
                  >
                    <Play size={18} fill="currentColor" /> Rozpocznij trening
                  </button>
                </>
              )}
            ...
            >
              {trainingHistory.length === 0 ? (
                <div className="text-center py-12 opacity-50 space-y-4">
                   ...
                   <p className="text-sm font-bold uppercase tracking-tight">Brak zapisanych treningów</p>
                </div>
              ) : (
                    ...
                    >
                          ...
                          <div className="flex-1 min-w-0">
                             ...
                             <div className="flex items-center gap-3 text-[10px] font-bold text-slate-500">
                                ...
                                <span className="flex items-center gap-1 uppercase tracking-tighter">
                                  {t.intensity === 'low' ? 'Lekka' : t.intensity === 'medium' ? 'Średnia' : 'Wysoka'}
                                </span>
                             </div>
                          </div>
                       ...
                       <div className="grid grid-cols-2 gap-2 mt-4">
                          <div className="bg-slate-50 dark:bg-white/5 p-2 rounded-2xl">
                             <div className="text-[8px] font-black uppercase text-slate-400 mb-0.5">Start</div>
                             <div className="text-xs font-black text-slate-700 dark:text-slate-200">{t.startSugar || '---'} <span className="text-[9px] opacity-60">mg/dL</span></div>
                          </div>
                          <div className="bg-slate-50 dark:bg-white/5 p-2 rounded-2xl">
                             <div className="text-[8px] font-black uppercase text-slate-400 mb-0.5">Koniec</div>
                             <div className="text-xs font-black text-slate-700 dark:text-slate-200">{t.endSugar || '---'} <span className="text-[9px] opacity-60">mg/dL</span></div>
                          </div>
                       </div>
                    ...
                    <div>
                      ...
                      <div className="space-y-6">
                        <div className={cn("p-5 rounded-3xl", isGlassmorphic ? "bg-white/40 dark:bg-white/5" : "bg-slate-50 dark:bg-slate-800/50")}>
                          <h4 className="text-[10px] font-black uppercase text-slate-500 mb-2 tracking-wider">Mechanizm Działania</h4>
                        ...
                        <div className={cn("p-5 rounded-3xl border", sport.bg, sport.border)}>
                          <h4 className={cn("text-[10px] font-black uppercase mb-3 tracking-wider", sport.color)}>Praktyczne Wskazówki</h4>
              ...
              >
                <div className="col-span-full mb-2">
                   <p className="text-sm font-medium text-slate-500 dark:text-slate-400">
                     Rozmaite wysiłki fizyczne mają różny wpływ na poziom glukozy. Wybierz dyscyplinę, aby dowiedzieć się na co uważać.
                   </p>
                </div>
