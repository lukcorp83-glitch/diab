...
export default function DietScoreWidget({ user, activeDiet }: { user: User, activeDiet: string }) {
  ...
  const getRecommendations = () => {
    switch (activeDiet) {
      case 'keto': return 'Utrzymujesz świetnie węglowodany, zjedz nieco więcej zdrowych tłuszczy np. awokado.';
      case 'dash': return 'Uwaga na sód! Wczorajsze posiłki zawierały sporo soli. Zwiększ potas.';
      case 'gluten': return 'Doskonałe wyniki bezglutenowe. Uważaj na niedobory błonnika.';
      case 'plate': return '2/3 Twoich posiłków w tym tygodniu miało idealne proporcje 50/25/25!';
      default: return 'Trzymasz się planu. Tak trzymaj!';
    }
  };
    ...
    <div className="bg-gradient-to-br from-indigo-50 to-blue-50 dark:from-indigo-900/20 dark:to-blue-900/20 rounded-2xl p-4 border border-indigo-100 dark:border-indigo-800/30">
       <div className="flex items-center gap-4 mb-4">
         ...
         <div className="flex-1">
           ...
           <p className="text-[10px] text-slate-600 dark:text-slate-400 font-medium leading-relaxed">
             Tygodniowy wskaźnik adherencji
           </p>
         </div>
       </div>
       ...
       <div className="bg-white/50 dark:bg-slate-900/50 rounded-xl p-3 mb-4">
         <p className="text-[10px] font-bold text-indigo-700 dark:text-indigo-300 leading-relaxed">
            <span className="font-black">GlikoSense: </span> 
            {getRecommendations()}
         </p>
       </div>
       ...
       <div className="grid grid-cols-2 gap-3">
         <div className="bg-white dark:bg-slate-800 rounded-xl p-3 text-center border border-slate-100 dark:border-slate-700/50">
           <span className="block text-[9px] font-black uppercase tracking-widest text-emerald-500 mb-1">Błonnik</span>
           <span className="text-lg font-black text-slate-900 dark:text-white">{fiber}g</span>
           <span className="text-[9px] text-slate-400 block mt-0.5">cel: 30g</span>
         </div>
         <div className="bg-white dark:bg-slate-800 rounded-xl p-3 text-center border border-slate-100 dark:border-slate-700/50">
           <span className="block text-[9px] font-black uppercase tracking-widest text-amber-500 mb-1">Sód (Na)</span>
           <span className="text-lg font-black text-slate-900 dark:text-white">{sodium}</span>
           <span className="text-[9px] text-slate-400 block mt-0.5">cel: &lt;2300mg</span>
         </div>
       </div>
    </div>
  );
