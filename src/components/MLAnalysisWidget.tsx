...
export default function MLAnalysisWidget({ logs, settings, user }: MLAnalysisWidgetProps) {
  ...
  const handleBackupConsentChange = (checked: boolean) => {
    ...
    if (checked) {
      toast.success(i18n.t('auto.zgoda_udzielona_mozesz_teraz_z', { defaultValue: "Zgoda udzielona. Możesz teraz zarządzać kopią zapasową." }));
    }
  };

  const handleBackupToCloud = async () => {
    if (!user || user.isAnonymous) {
      toast.error(i18n.t('auto.zaloguj_sie_na_pelne_konto_e_m', { defaultValue: "Zaloguj się na pełne konto (E-mail lub Google), aby korzystać z kopii zapasowej." }));
      return;
    }
    if (!hasBackupConsent) {
      toast.error(i18n.t('auto.musisz_najpierw_zaakceptowac_i', { defaultValue: "Musisz najpierw zaakceptować informację o zgodzie." }));
      return;
    }
    ...
    try {
      ...
      toast.success(i18n.t('auto.kopia_zapasowa_modelu_glikosen', { defaultValue: "Kopia zapasowa modelu GlikoSense 3.0 została zapisana pomyślnie!" }), { id: toastId });
    } catch (err) {
      toast.error(i18n.t('auto.blad_podczas_eksportowania_lub', { defaultValue: "Błąd podczas eksportowania lub zapisu kopii zapasowej." }), { id: toastId });
  ...
  const handleRestoreFromCloud = async () => {
    if (!user || user.isAnonymous) {
      toast.error(i18n.t('auto.zaloguj_sie_na_pelne_konto_e_m', { defaultValue: "Zaloguj się na pełne konto (E-mail lub Google), aby pobrać kopię zapasową." }));
      return;
    }
    if (!hasBackupConsent) {
      toast.error(i18n.t('auto.udziel_najpierw_zgody_na_zarza', { defaultValue: "Udziel najpierw zgody na zarządzanie kopią zapasową." }));
      return;
    }

    const confirmRestore = window.confirm(
      i18n.t('auto.uwaga_przywrocenie_modelu_z_ch', { defaultValue: "UWAGA: Przywrócenie modelu z chmury CAŁKOWICIE nadpisze obecne lokalne parametry sieci neuronowej GlikoSense 3.0 zainstalowane w przeglądarce. Czy chcesz kontynuować?" })
    );
    ...
    try {
      ...
      if (success) {
        toast.success(i18n.t('auto.model_glikosense_3_0_zostal_po', { defaultValue: "Model GlikoSense 3.0 został pomyślnie przywrócony z chmury!" }), { id: toastId });
        runML(true);
      } else {
        toast.error(i18n.t('auto.wystapil_nieznany_problem_z_pl', { defaultValue: "Wystąpił nieznany problem z plikiem modelu." }), { id: toastId });
      }
    } catch (err) {
      toast.error(i18n.t('auto.blad_podczas_przywracania_kopi', { defaultValue: "Błąd podczas przywracania kopii zapasowej." }), { id: toastId });
  ...
  const handleDeleteBackup = async () => {
    ...
    const confirmDelete = window.confirm(i18n.t('auto.czy_na_pewno_chcesz_usunac_kop', { defaultValue: "Czy na pewno chcesz usunąć kopię zapasową modelu z chmury? Ta operacja jest nieodwracalna." }));
    ...
    try {
      ...
      setBackupInfo(null);
      toast.success(i18n.t('auto.kopia_zapasowa_w_chmurze_zosta', { defaultValue: "Kopia zapasowa w chmurze została usunięta." }), { id: toastId });
    } catch (err) {
      toast.error(i18n.t('auto.blad_podczas_usuwania_kopii_za', { defaultValue: "Błąd podczas usuwania kopii zapasowej." }), { id: toastId });
  ...
  const handleExportToFile = async () => {
    try {
      const modelData = await MLAnalyzer.exportCurrentModel();
      if (!modelData) {
        toast.error(i18n.t('auto.brak_wytrenowanego_modelu_do_p', { defaultValue: "Brak wytrenowanego modelu do pobrania. Wykonaj najpierw analizę!" }));
        return;
      }
      ...
      toast.success(i18n.t('auto.pomyslnie_pobrano_model_do_pli', { defaultValue: "Pomyślnie pobrano model do pliku JSON!" }));
    } catch (err) {
      toast.error(i18n.t('auto.blad_eksportu_do_pliku', { defaultValue: "Błąd eksportu do pliku." }));
      console.error(err);
    }
  };
    ...
    fileReader.onload = async (event) => {
      try {
        ...
        if (!parsed.modelTopology || !parsed.weightSpecs || !parsed.weightDataB64) {
          throw new Error(i18n.t('auto.nieprawidlowy_format_pliku_mod', { defaultValue: "Nieprawidłowy format pliku modelu GlikoSense." }));
        }
        
        const confirmRestore = window.confirm(
          i18n.t('auto.uwaga_wgranie_modelu_z_pliku_n', { defaultValue: "Uwaga: Wgranie modelu z pliku nadpisze aktualny model w przeglądarce. Czy chcesz kontynuować?" })
        );
        if (!confirmRestore) return;
        
        const success = await MLAnalyzer.importModelFromBackup(parsed);
        if (success) {
          toast.success(i18n.t('auto.pomyslnie_wgrano_model_z_pliku', { defaultValue: "Pomyślnie wgrano model z pliku JSON!" }));
          runML(true);
        } else {
          toast.error(i18n.t('auto.blad_podczas_importu_modelu', { defaultValue: "Błąd podczas importu modelu." }));
        }
      } catch (err) {
        toast.error(err instanceof Error ? err.message : i18n.t('auto.blad_odczytu_pliku_upewnij_sie', { defaultValue: "Błąd odczytu pliku. Upewnij się, że plik jest poprawnym JSONem modelu." }));
        console.error(err);
      } finally {
        e.target.value = '';
      }
    ...
    try {
        ...
        MLAnalyzer.analyzeData(logs, force, 'full')
          ...
          .catch(e => {
              ...
              if (!qResult) setError(i18n.t('auto.blad_pelnej_analizy', { defaultValue: "Błąd pełnej analizy." }));
          })
          .finally(() => {
        ...
        setError(i18n.t('auto.nie_udalo_sie_przeanalizowac_d', { defaultValue: "Nie udało się przeanalizować danych. Spróbuj później." }));
        clearTimeout(safetyTimeout);
        setIsAnalyzing(false);
    }
  };
    ...
    <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl p-6 md:p-8 rounded-[2.5rem] shadow-2xl border border-accent-100 dark:border-accent-900/40 relative overflow-hidden group">
      ...
      <div className="flex items-center justify-between mb-8 relative z-10">
        ...
        <button 
          ...
          title={t('auto.odśwież_analizę', { defaultValue: 'Odśwież analizę' })}
        >
      ...
      <div className="relative z-20 mb-6 bg-slate-50 dark:bg-slate-800/20 p-4 border border-slate-200/40 dark:border-slate-800/40 rounded-[2rem] hover:border-indigo-500/20 transition-all">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div className="flex flex-col">
              <span className="text-sm font-black text-slate-700 dark:text-slate-100 leading-none">{t('auto.sieć_neuronowa_glikosense_3_0', { defaultValue: 'Sieć neuronowa GlikoSense 3.0' })}</span>
          ...
          >
            {showBackupPanel ? i18n.t('auto.zwin', { defaultValue: "Zwiń" }) : i18n.t('auto.zarzadzaj', { defaultValue: "Zarządzaj" })}
          </button>
        </div>
          ...
          >
            ...
            <div className="bg-amber-500/5 hover:bg-amber-500/10 border border-amber-500/20 p-4 rounded-2xl flex gap-3 text-amber-700 dark:text-amber-400">
              ...
              <div className="space-y-1">
                <span className="text-xs font-black uppercase tracking-wider block">{t('auto.ważna_informacja_o_modelu', { defaultValue: 'Ważna Informacja o Modelu' })}</span>
                <p className="text-xs font-medium leading-relaxed">
                  
                                                    {t('auto.twoja_sieć_neuronowa_uczy_się_lokal', { defaultValue: 'Twoja sieć neuronowa uczy się lokalnie na Twoim urządzeniu. Czyszczenie pamięci podręcznej przeglądarki lub zmiana urządzenia spowoduje' })}{" "}
                  <strong className="font-extrabold text-amber-600 dark:text-amber-300">{t('auto.bezzwrotną_utratę_wyuczonego_modelu', { defaultValue: 'bezzwrotną utratę wyuczonego modelu GlikoSense 3.0' })}</strong>{" "}
                  
                                                    {t('auto.i_przywrócenie_wartości_podstawowyc', { defaultValue: 'i przywrócenie wartości podstawowych. Kopia w chmurze chroni przed utratą Twojej spersonalizowanej inteligencji.' })}
                                                  </p>
              </div>
            </div>
            ...
            >
              ...
              <div className="space-y-0.5">
                <span className="text-xs font-bold text-slate-700 dark:text-slate-200">
                                                    ...
                                                    {t('auto.rozumiem_ryzyko_i_wyrażam_świadomą_', { defaultValue: 'Rozumiem ryzyko i wyrażam świadomą zgodę' })}
                                                  </span>
                <p className="text-[10px] text-slate-400">
                  
                                                    {t('auto.wyrażam_zgodę_na_bezpieczny_szyfrow', { defaultValue: 'Wyrażam zgodę na bezpieczny, szyfrowany zapis wag i topologii mojej lokalnej sieci neuronowej w moim profilu bazy danych Firebase.' })}
                                                  </p>
              </div>
            </div>
            ...
            {!user ? (
              <div className="bg-indigo-500/5 border border-indigo-500/15 p-3 rounded-2xl text-center text-xs text-indigo-500 font-bold">
                                              ...
                                              {t('auto.zaloguj_się_na_pełne_konto_e_mailem', { defaultValue: '⚠️ Zaloguj się na pełne konto (e-mailem lub Google), aby uzyskać dostęp do kopii zapasowej w bezpiecznej chmurze.' })}
                                            </div>
            ) : user.isAnonymous ? (
              <div className="bg-amber-500/5 border border-amber-500/15 p-3 rounded-2xl text-center text-xs text-amber-600 dark:text-amber-400 font-bold">
                                                  ...
                                                  {t('auto.kopia_zapasowa_modelu_jest_niedostę', { defaultValue: '⚠️ Kopia zapasowa modelu jest niedostępna w trybie gościa. Zapobiegaj utracie modelu logując się na pełne konto (E-mail lub Google).' })}
                                                </div>
            ) : (
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  >
                    <CloudDownload size={14} />  {t('auto.przywróć_z_chmury', { defaultValue: 'PRZYWRÓĆ Z CHMURY' })}
                                                                </button>
                  ...
                  {backupInfo && (
                    <button
                      ...
                      title={t('auto.usuń_kopię_zapasową_z_chmury', { defaultValue: 'Usuń kopię zapasową z chmury' })}
                    >
                      <Trash2 size={14} />
                    </button>
                  )}
            ...
            <div className="pt-3.5 border-t border-slate-200/40 dark:border-slate-800/40 space-y-3">
              <div className="flex flex-col">
                <span className="text-[10px] font-black uppercase text-slate-400 dark:text-slate-500 tracking-wider">{t('auto.kopia_lokalna_plik_idealne_dla_plik', { defaultValue: 'Kopia Lokalna / Plik (Idealne dla pliku APK & Gości)' })}</span>
                <p className="text-[10px] text-slate-400 leading-tight mt-0.5">
                  
                                                    {t('auto.całkowicie_bezpłatne_pobieranie_spe', { defaultValue: 'Całkowicie bezpłatne pobieranie spersonalizowanych wag sieci neuronowej bezpośrednio na pamięć urządzenia lub ich wczytywanie z pliku JSON.' })}
                                                  </p>
              </div>
      ...
      <AnimatePresence mode="wait">
          {logs.filter(l => l.type === 'glucose' || l.bg).length < 5 ? (
            ...
            >
               <span className="text-xs font-bold uppercase tracking-widest text-slate-400">{t('auto.zbyt_mało_danych_do_analizy_min_5_w', { defaultValue: 'Zbyt mało danych do analizy (min. 5 wpisów)' })}</span>
            </motion.div>
          ) : error && !mlResult ? (
            <motion.div 
               key="error"
            ...
            >
               ...
               >
                                               ...
                                               {t('auto.spróbuj_ponownie', { defaultValue: 'Spróbuj ponownie' })}
                                             </button>
            </motion.div>
                      ...
                      <div className="flex flex-col gap-3 md:gap-5">
                          <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-md p-5 md:p-8 rounded-[2.5rem] border border-slate-200/60 dark:border-slate-700/60 shadow-lg flex-1 flex flex-col justify-center relative overflow-hidden group">
                              ...
                              <div className="flex items-center gap-2 mb-2 md:mb-3 relative z-10">
                                  ...
                                  <span className="text-[9px] md:text-[10px] font-black text-slate-500 uppercase tracking-[0.1em] md:tracking-[0.2em] leading-tight">{t('auto.pewność_modelu', { defaultValue: 'Pewność Modelu' })}</span>
                              </div>
                  ...
                  {mlResult.metrics && (
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                          <div className="bg-white dark:bg-slate-800/60 p-4 rounded-3xl border border-slate-200/50 dark:border-slate-700/50 shadow-sm hover:shadow-md transition-shadow group glass-target">
                              <span className="block text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1.5 group-hover:text-indigo-500 transition-colors">{t('auto.profil_działania_insuliny', { defaultValue: 'Profil Działania Insuliny' })}</span>
                              <div className="flex flex-col">
                          ...
                          <div className="bg-white dark:bg-slate-800/60 p-4 rounded-3xl border border-slate-200/50 dark:border-slate-700/50 shadow-sm hover:shadow-md transition-shadow group glass-target">
                              <span className="block text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1.5 group-hover:text-amber-500 transition-colors">{t('auto.aktywne_węglow', { defaultValue: 'Aktywne Węglow.' })}</span>
                          ...
                          <div className="bg-white dark:bg-slate-800/60 p-4 rounded-3xl border border-slate-200/50 dark:border-slate-700/50 shadow-sm hover:shadow-md transition-shadow group glass-target">
                              <span className="block text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1.5 group-hover:text-amber-500 transition-colors">{t('auto.oporność_bias', { defaultValue: 'Oporność (Bias)' })}</span>
                          ...
                          <div className="bg-white dark:bg-slate-800/60 p-4 rounded-3xl border border-slate-200/50 dark:border-slate-700/50 shadow-sm hover:shadow-md transition-shadow group glass-target">
                              <span className="block text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1.5 group-hover:text-emerald-500 transition-colors">{t('auto.gmi_wskaźnik', { defaultValue: 'GMI (Wskaźnik)' })}</span>
                          ...
                          <div className="bg-white dark:bg-slate-800/60 p-4 rounded-3xl border border-slate-200/50 dark:border-slate-700/50 shadow-sm hover:shadow-md transition-shadow group glass-target">
                              <span className="block text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1.5 group-hover:text-orange-500 transition-colors">{t('auto.czułość_węg', { defaultValue: 'Czułość (Węg.)' })}</span>
                              <span className="text-2xl font-black text-slate-800 dark:text-slate-100 tracking-tighter">
                          ...
                          <div className="bg-white dark:bg-slate-800/60 p-4 rounded-3xl border border-slate-200/50 dark:border-slate-700/50 shadow-sm hover:shadow-md transition-shadow group glass-target">
                              <span className="block text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1.5 group-hover:text-cyan-500 transition-colors">{t('auto.wrażliwość_ins', { defaultValue: 'Wrażliwość (Ins.)' })}</span>
                              <span className="text-2xl font-black text-slate-800 dark:text-slate-100 tracking-tighter">
