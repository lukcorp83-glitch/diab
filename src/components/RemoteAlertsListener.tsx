...
export default function RemoteAlertsListener({ }: { user: any }) {
  const user = useAuthStore(state => state.user);

 const { t } = useTranslation();
 const [activeAlert, setActiveAlert] = useState<RemoteAlert | null>(null);

 const { data: alerts = [] } = useAlerts(user);
 ...
 notifications: [
 {
 title: i18n.t('auto.wiadomosc_od', { defaultValue: 'Wiadomość od: ' }) + (newAlert as any).senderDevice,
 body: (newAlert as any).message,
 id: new Date().getTime(),
 ...
 setActiveAlert(newAlert as RemoteAlert);
 }, [alerts]);
