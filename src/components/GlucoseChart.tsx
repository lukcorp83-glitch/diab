...
export default function GlucoseChart({ hours, targetMin, targetMax, theme, settings, showLoopSimulation, showMLPrediction }: GlucoseChartProps) {
 ...
 const iob = logs
 .filter(l => (l.type === 'bolus' || (l.type as any) === 'insulin') && nowMs - l.timestamp < diaMs && nowMs - l.timestamp >= 0)
 .reduce((sum, b) => {
 ...
 const cob = logs
 .filter(l => (l.type === 'meal' || ((l.type === 'bolus' || (l.type as any) === 'insulin') && l.linkedMeal?.carbs)) && nowMs - l.timestamp < digestionMs && nowMs - l.timestamp >= 0)
 .reduce((sum, m) => {
 ...
 stackingWarning: stackingEvents.has(d.timestamp)
   ...
   const carbsVal = d.linkedMeal?.carbs || (d as any).carbs;
   if (carbsVal > 0) {
   addPoint(d.timestamp, 'mealVal', true, { 
   originalM: { ...d, type: 'meal', value: carbsVal }, 
   mealY: chartMinY 
   });
   }
 });
