...
self.onmessage = async (e: MessageEvent<GlikoWorkerInput>) => {
  ...
  try {
    ...
    if (carbSensitivity > 30) insights.push(`💡 Wydaje mi się, że węglowodany dość szybko wchłaniają się teraz u Ciebie do krwi. Porcja np. 50g może mieć duży wpływ!`);
    else if (carbSensitivity > 10) insights.push(`💡 To co zjadłeś, uwalnia się moim zdaniem bardzo spokojnie i bez skoków. Dobra nasza!`);

    if (insulinSensitivity < -30) insights.push(`💉 Masz w tym momencie podwyższoną wrażliwość na insulinę. Postaraj się delikatniej podejść do ewentualnych korekt.`);
    ...
    else if (avgBias > 15) insights.push(`🏃‍♂️ Cukier trzyma się niżej niż przewidywałem! Miałeś ukryty wysiłek fizyczny, o którym mi nie powiedziałeś?`);
    ...
    if (rules.somogyiEnabled && recentHypos.length > 0 && lastTrendNum > 3) {
        insights.push(`🔄 Uwaga! Ostatnio miałeś hipo, więc widzę jak Twój organizm zaczął walczyć wypuszczając zapasy glukozy (tzw. efekt Somogyi). Powoli z korektami.`);
    } else if (recentHypos.length > 0 && lastTrendNum > 6) {
        insights.push(`🔄 Miałeś przed chwilą głęboki spadek. Ten gwałtowny wzrost to odbicie po-hipowe. Uważnie z potężną korektą, by znów nie spaść!`);
    }

    if (rules.insulinResistanceMultiplier && rules.insulinResistanceMultiplier > 1.1) insights.push(`💪 W moim cenniku Twojego uodpornienia, widnieje lekka blokada na insulinę. Licz się z trochę chłodniejszą reakcją organizmu.`);
    else if (rules.insulinResistanceMultiplier && rules.insulinResistanceMultiplier < 0.9) insights.push(`📉 Moje ukryte wagi mówią, że powinieneś w najbliższym czasie być podwójnie ostrożny z dawkami. Będziesz zbijał cukier skuteczniej!`);
    ...
    if (countGlucose > 10) {
        ...
        if (latestBg > bollingerUpper && lastTrendNum > 3) insights.push(`📈 Oho, poszliśmy wyżej niż zakładałem! Cukier chyba poszukuje nowego miejsca żeby odpocząć, musisz zareagować.`);
        else if (latestBg < bollingerLower && lastTrendNum < -3) insights.push(`📉 Cukier zapikował ostrzej niż można by sądzić! Polecam szybko zabezpieczyć się węglowodanami.`);

        if (cv > 33 && meanGlucose > 80 && countGlucose > 30) insights.push(`🎢 Hej, jedziemy na sporym roller-coasterze. Zwróć większą uwagę na wcześniejsze podanie insuliny by ograniczyć piki!`);
        else if (cv < 15 && countGlucose > 30) insights.push(`🏆 Chciałbym Ci bardzo podziękować - Twoje wykresy trzymają tak mało huśtawek, to powód do zadowolenia!`);
    }
    ...
    if (currentFob > 15 || currentPob > 20) insights.push(`⚠️ Mam u siebie zapisaną masę tłuszczów lub białek czekającą by się strawić! Zwracaj uwagę na ukryte wyskoki potraw za ok. 3-4 godziny - efekt pizzy wisi w powietrzu!`);
