...
function calculateActiveAtTime(targetTime: number, pastLogs: any[], rules: any) {
    ...
    const classifyMealGlycemia = (meal: any) => {
        ...
        const fastKeywords = ["sok", "cukier", "glukoza", "glucose", i18n.t('auto.zel', { defaultValue: i18n.t('auto.zel', { defaultValue: i18n.t('auto.zel', { defaultValue: "żel" }) }) }), "dextro", i18n.t('auto.miod', { defaultValue: i18n.t('auto.miod', { defaultValue: i18n.t('auto.miod', { defaultValue: "miód" }) }) }), "honey", "cola", i18n.t('auto.slodkie', { defaultValue: i18n.t('auto.slodkie', { defaultValue: i18n.t('auto.slodkie', { defaultValue: "słodkie" }) }) }), i18n.t('auto.slodki', { defaultValue: i18n.t('auto.slodki', { defaultValue: i18n.t('auto.slodki', { defaultValue: "słodki" }) }) }), i18n.t('auto.zelki', { defaultValue: i18n.t('auto.zelki', { defaultValue: i18n.t('auto.zelki', { defaultValue: "żelki" }) }) }), "banan", i18n.t('auto.dzem', { defaultValue: i18n.t('auto.dzem', { defaultValue: i18n.t('auto.dzem', { defaultValue: "dżem" }) }) }), "sprite", "fanta", i18n.t('auto.oranzada', { defaultValue: i18n.t('auto.oranzada', { defaultValue: i18n.t('auto.oranzada', { defaultValue: "oranżada" }) }) }), "herbata z cukrem", "cukierki", "czekolada", "owoce", "juice"];
        const slowKeywords = ["pizza", "kebab", "burger", "ser", "cheese", "orzechy", i18n.t('auto.mieso', { defaultValue: i18n.t('auto.mieso', { defaultValue: i18n.t('auto.mieso', { defaultValue: "mięso" }) }) }), "meat", "pasta", "spaghetti", "makaron", "boczek", "frytki", i18n.t('auto.maslo', { defaultValue: i18n.t('auto.maslo', { defaultValue: i18n.t('auto.maslo', { defaultValue: "masło" }) }) }), i18n.t('auto.tluszcz', { defaultValue: i18n.t('auto.tluszcz', { defaultValue: i18n.t('auto.tluszcz', { defaultValue: "tłuszcz" }) }) }), i18n.t('auto.bialko', { defaultValue: i18n.t('auto.bialko', { defaultValue: i18n.t('auto.bialko', { defaultValue: "białko" }) }) }), i18n.t('auto.karkowka', { defaultValue: i18n.t('auto.karkowka', { defaultValue: i18n.t('auto.karkowka', { defaultValue: "karkówka" }) }) }), i18n.t('auto.kielbasa', { defaultValue: i18n.t('auto.kielbasa', { defaultValue: i18n.t('auto.kielbasa', { defaultValue: "kiełbasa" }) }) }), "nuts", "chocolate"];
...
self.onmessage = async (e: MessageEvent<GlikoWorkerInput>) => {
  ...
  try {
    ...
    const timeBlocks = {
      ...
      evening: { label: i18n.t('auto.wieczor', { defaultValue: i18n.t('auto.wieczor', { defaultValue: i18n.t('auto.wieczor', { defaultValue: "Wieczór" }) }) }), starts: 17, ends: 23, sensitivity: 0, count: 0, drops: [] as number[] },
    ...
    if (glucoseLogsOrig.length < 5) {
      self.postMessage({ type: 'result', payload: { predictedNextHour: 0, predictedNext2Hours: 0, riskOfHypo: false, insights: [...insights, i18n.t('auto.czekam_na_wiecej_odczytow_glik', { defaultValue: i18n.t('auto.czekam_na_wiecej_odczytow', { defaultValue: i18n.t('auto.czekam_na_wiecej_odczytow', { defaultValue: "Czekam na więcej odczytów glikemii..." }) }) })], accuracy: 0, analyzedPeriod: mode === 'quick' ? 'Ostatnie 4h' : 'Ostatnie 14 dni' } });
      return;
    }
    ...
    if (resampledGlucose.length === 0) {
      self.postMessage({ type: 'result', payload: { predictedNextHour: 0, predictedNext2Hours: 0, riskOfHypo: false, insights: [i18n.t('auto.zbyt_malo_poprawnych_danych_gl', { defaultValue: i18n.t('auto.zbyt_malo_poprawnych_dany', { defaultValue: i18n.t('auto.zbyt_malo_poprawnych_dany', { defaultValue: "Zbyt mało poprawnych danych glikemii po przetworzeniu." }) }) })], accuracy: 0 } });
      return;
    }
    ...
    const accuracyPhrases = [
        i18n.t('auto.oparlem_sie_o_moje_doswia', { defaultValue: "🧠 Oparłem się o moje doświadczenie z Twoich ostatnich dni. Mój margines błędu to około {{var0}} mg/dL.", var0: Math.round(avgErrorInMgDl) }).replace('{{var0}}', String(Math.round(avgErrorInMgDl))),
        i18n.t('auto.wciaz_zbieram_dane_moje_o', { defaultValue: "🧠 Wciąż zbieram dane. Moje odchylenie na tę chwilę to ok. {{var0}} mg/dL. Im więcej danych, tym mniejszy błąd.", var0: Math.round(avgErrorInMgDl) }).replace('{{var0}}', String(Math.round(avgErrorInMgDl))),
        i18n.t('auto.przeanalizowalem_twoje_wy', { defaultValue: "🧠 Przeanalizowałem Twoje wykresy używając silnika LSTM. Błąd w przewidywaniach to {{var0}} mg/dL.", var0: Math.round(avgErrorInMgDl) }).replace('{{var0}}', String(Math.round(avgErrorInMgDl)))
    ];
    insights.push(accuracyPhrases[Math.floor(Math.random() * accuracyPhrases.length)]);

    const predictionPhrases = [
        i18n.t('auto.jezeli_trend_sie_nie_zmie', { defaultValue: "🔮 Jeżeli trend się nie zmieni, w ciągu godziny powinniśmy dotrzeć do {{var0}} mg/dL.", var0: Math.round(predictedNextHour) }).replace('{{var0}}', String(Math.round(predictedNextHour))),
        i18n.t('auto.wybiegajac_do_przodu_prze', { defaultValue: "🔮 Wybiegając do przodu - przewiduję okolice {{var0}} mg/dL za godzinkę.", var0: Math.round(predictedNextHour) }).replace('{{var0}}', String(Math.round(predictedNextHour))),
        i18n.t('auto.analizujac_tempo_spodziew', { defaultValue: "🔮 Analizując tempo, spodziewam się poziomu ok. {{var0}} mg/dL w ciągu 60 minut.", var0: Math.round(predictedNextHour) }).replace('{{var0}}', String(Math.round(predictedNextHour)))
    ];
    ...
    if (carbSensitivity > 30) insights.push(i18n.t('auto.wydaje_mi_sie_ze_weglowodany_d', { defaultValue: i18n.t('auto.wydaje_mi_sie_ze_weglowod', { defaultValue: i18n.t('auto.wydaje_mi_sie_ze_weglowod', { defaultValue: "💡 Wydaje mi się, że węglowodany dość szybko wchłaniają się teraz u Ciebie do krwi. Porcja np. 50g może mieć duży wpływ!" }) }) }));
    else if (carbSensitivity > 10) insights.push(i18n.t('auto.to_co_zjadles_uwalnia_sie_moim', { defaultValue: i18n.t('auto.to_co_zjadles_uwalnia_sie', { defaultValue: i18n.t('auto.to_co_zjadles_uwalnia_sie', { defaultValue: "💡 To co zjadłeś, uwalnia się moim zdaniem bardzo spokojnie i bez skoków. Dobra nasza!" }) }) }));
    ...
    if (rules.dawnPhenomenonEnabled && lastHourDec > 2 && lastHourDec < 9 && lastTrendNum > 3) {
         insights.push(i18n.t('auto.poranne_wstawanie_twoj_organiz', { defaultValue: i18n.t('auto.poranne_wstawanie_twoj_or', { defaultValue: i18n.t('auto.poranne_wstawanie_twoj_or', { defaultValue: "🌅 Poranne wstawanie... Twój organizm włącza Zjawisko Brzasku. Zapisuję, że w ostatnich dniach u Ciebie to częste." }) }) }));
    } else if (lastHourDec > 3 && lastHourDec < 8 && lastTrendNum > 5 && currentCob === 0 && currentPob === 0) {
        insights.push(i18n.t('auto.prawdopodobnie_organizm_budzi', { defaultValue: i18n.t('auto.prawdopodobnie_organizm_b', { defaultValue: i18n.t('auto.prawdopodobnie_organizm_b', { defaultValue: "🌅 Prawdopodobnie organizm budzi Cię właśnie hormonami porannymi. Tzw. zjawisko brzasku, rośniemy nawet bez jedzenia." }) }) }));
    }
    ...
    if (rules.somogyiEnabled && recentHypos.length > 0 && lastTrendNum > 3) {
        insights.push(i18n.t('auto.uwaga_ostatnio_miales_hipo_wie', { defaultValue: i18n.t('auto.uwaga_ostatnio_miales_hip', { defaultValue: i18n.t('auto.uwaga_ostatnio_miales_hip', { defaultValue: "🔄 Uwaga! Ostatnio miałeś hipo, więc widzę jak Twój organizm zaczął walczyć wypuszczając zapasy glukozy (tzw. efekt Somogyi). Powoli z korektami." }) }) }));
    ...
    if (latestBg > 160 && currentIob > 1.5 && predictedNextHour > 160) {
        insights.push(i18n.t('auto.jest_wysoko_ale_uwaga_ustrzez', { defaultValue: i18n.t('auto.jest_wysoko_ale_uwaga_ust', { defaultValue: i18n.t('auto.jest_wysoko_ale_uwaga_ust', { defaultValue: "📉 Jest wysoko, ale uwaga! Ustrzeż się nawarstwienia insuliny, masz już zgromadzoną u siebie aktywną porcję. Wymaga to tylko chwili cierpliwości." }) }) }));
    }

    if ((lastHourDec > 23 || lastHourDec < 5) && Math.abs(lastTrendNum) < 3 && latestBg > 90 && latestBg < 140) {
        insights.push(i18n.t('auto.widze_spokojna_i_udana_nocke_p', { defaultValue: i18n.t('auto.widze_spokojna_i_udana_no', { defaultValue: i18n.t('auto.widze_spokojna_i_udana_no', { defaultValue: "🌙 Widzę spokojną i udaną nockę. Parametry ucięte jak od sznurka do snu!" }) }) }));
    }
    ...
    if (countGlucose > 10) {
        ...
        if (latestBg > bollingerUpper && lastTrendNum > 3) insights.push(i18n.t('auto.oho_poszlismy_wyzej_niz_zaklad', { defaultValue: i18n.t('auto.oho_poszlismy_wyzej_niz_z', { defaultValue: i18n.t('auto.oho_poszlismy_wyzej_niz_z', { defaultValue: "📈 Oho, poszliśmy wyżej niż zakładałem! Cukier chyba poszukuje nowego miejsca żeby odpocząć, musisz zareagować." }) }) }));
        else if (latestBg < bollingerLower && lastTrendNum < -3) insights.push(i18n.t('auto.cukier_zapikowal_ostrzej_niz_m', { defaultValue: i18n.t('auto.cukier_zapikowal_ostrzej', { defaultValue: i18n.t('auto.cukier_zapikowal_ostrzej', { defaultValue: "📉 Cukier zapikował ostrzej niż można by sądzić! Polecam szybko zabezpieczyć się węglowodanami." }) }) }));

        if (cv > 33 && meanGlucose > 80 && countGlucose > 30) insights.push(i18n.t('auto.hej_jedziemy_na_sporym_roller', { defaultValue: i18n.t('auto.hej_jedziemy_na_sporym_ro', { defaultValue: i18n.t('auto.hej_jedziemy_na_sporym_ro', { defaultValue: "🎢 Hej, jedziemy na sporym roller-coasterze. Zwróć większą uwagę na wcześniejsze podanie insuliny by ograniczyć piki!" }) }) }));
        else if (cv < 15 && countGlucose > 30) insights.push(i18n.t('auto.chcialbym_ci_bardzo_podziekowa', { defaultValue: i18n.t('auto.chcialbym_ci_bardzo_podzi', { defaultValue: i18n.t('auto.chcialbym_ci_bardzo_podzi', { defaultValue: "🏆 Chciałbym Ci bardzo podziękować - Twoje wykresy trzymają tak mało huśtawek, to powód do zadowolenia!" }) }) }));
    }
