...
data = data.replace(
  /className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 (p-3|py-3|p-4)( .*?)rounded-xl/g,
...
data = data.replace(/text-slate-500/g, 'text-slate-500 dark:text-slate-400');
...
// In some cases we might have introduced dark:text-slate-400 in text that already had dark:text-white
data = data.replace(/text-slate-500 dark:text-slate-400 dark:text-white/g, 'text-slate-500 dark:text-slate-400'); // if it was text-slate-500 dark:text-white, well replacing it might be fine, or let's be careful. Let's just fix text-slate-500 that didn't have dark.
...
data2 = data2.replace(/text-slate-500/g, 'text-slate-500 dark:text-slate-400');
...
data3 = data3.replace(/text-slate-500/g, 'text-slate-500 dark:text-slate-400');
...
if(fs.existsSync('src/components/MealEditModal.tsx')){
  ...
  data4 = data4.replace(/text-slate-500/g, 'text-slate-500 dark:text-slate-400');
