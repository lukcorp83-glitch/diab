...
- `src/constants.ts` - Główne stałe, w tym `APP_VERSION` ('6.0.38'), adresy URL oraz bazy produktów.
- `src/constants/versions.ts` - Logika wersji (PWA, APK), definicje okien z historią nowości (`whatsNew`). Zaktualizowano do v6.0.38.
...
- `src/services/preBolusService.ts` & `android/app/src/main/java/com/glikocontrol/app/NightscoutFetcher.java` & `NotificationBridgePlugin.java` - Automatyczny stoper przedposiłkowy z twardą blokadą wartości ujemnych (`Math.max(0, ...)`), natychmiastowym przejściem w powiadomienie 'Czas na posiłek!' (z wyłączeniem chronometru), pętlą odświeżania cyfry minut na pasku stanu Androida co przejście minuty / 15s oraz synchronizacją w `DynamicActionCapsule.tsx`.
...
- `public/CNAME` & `version.json` - Wdrożono oficjalną domenę `glikocontrol.pl` z pełnym wsparciem Cloudflare SSL/HSTS oraz metatagami SEO w `index.html`. Usunięto nieużywaną bibliotekę `@capgo/capacitor-updater` – aktualizacje bazują bezpośrednio na pobieraniu instalatora APK.
...
- `src/components/nutrition/GlikoSenseNutriView.tsx` & `src/components/nutrition/NutritionHub.tsx` - Nowoczesny moduł Odżywiania z dynamicznymi kartami posiłków w stylu Bento. Zawiera wygładzone mini-krzywe glikemii (sparklines 0h-3h), animowane paski stabilności, pigułki makroskładników (W, B, T), wskaźnik spójności oraz przycisk 1-Click „Wrzuć na Talerz” przekazujący posiłek wprost do Kalkulatora/Talerza. Pełne i spójne wsparcie dla wszystkich motywów: Domyślny, Ciemny, AMOLED, Glassmorphism (`data-glassmorphism`) oraz Material Design 3 (`data-material3`).
