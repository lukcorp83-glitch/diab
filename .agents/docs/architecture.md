...
- **Szybkie Skróty (Shortcuts):** W przeciwieństwie do podstawowych ustawień, skróty posiłków i akcji są zapisywane w **osobnej kolekcji Firebase** (`users/{uid}/shortcuts`). Należy pamiętać o tej kolekcji przy wszelkich skryptach migracyjnych (np. `MigrationManager.tsx`), ponieważ nie znajdują się one wewnątrz dokumentu `settings/profile`.
- **Tryb Eko i Dynamiczne Kolory:** Tryb Eko redukuje zapytania Firebase z 2000 do 500 wpisów (w `useAppSubscriptions.ts`), ale zależy od flagi `ecoMode` zapisanej w `localStorage` (musi być synchronizowana ze stanem Firebase!). Oprócz redukcji transferu, Tryb Eko wyłącza animacje (Framer Motion) i ustawia atrybut `data-eco="true"`, co poprzez `index.css` ukrywa cienie i rozmycia. Przełącznik "Dynamiczne Kolory" (bez natywnego pluginu Capacitor) służy jako pseudo-dynamiczny fallback, który rotuje akcenty z `custom_colors.css` w oparciu o aktualny dzień roku.
...
- **Framer Motion `drag="x"` Dead Clicks**: Używanie `drag="x"` na głównym kontenerze (np. w `App.tsx` w celu nawigacji swipe'ami) powoduje przechwytywanie i "połykanie" zdarzeń kliknięć na głęboko zagnieżdżonych checkboxach i przyciskach. Przeglądarka interpretuje kliknięcia jako początek przeciągania. Unikaj globalnego `drag` na zawartości interaktywnej.
- **Recharts `ResponsiveContainer` Invisible Overlay**: Komponent `ResponsiveContainer` z biblioteki `recharts` tworzy ukrytą warstwę do mierzenia wielkości okna. Jeśli kontener wykresu nie ma zdefiniowanego `position: relative`, ta ukryta warstwa może wylać się na resztę ekranu i całkowicie zablokować kliknięcia (działając jak szyba). Zawsze owijaj `ResponsiveContainer` w `div` z klasą `relative z-0`, a klikalne elementy pod nim zabezpieczaj klasą `relative z-10`.
