...
async function testLogin() {
    ...
    let currentUrl = (initResponse.request)?.res?.responseUrl || initResponse.config.url || baseUrl;
    ...
    for (const inputMatch of inputs) {
        ...
        console.log(`Found ${tagType}: name=${nameMatch ? nameMatch[1] : null}, value=${valueMatch ? valueMatch[1] : null}`);
        ...
        if (tagType === 'button' && /type=["']submit["']/i.test(tagAttrs)) {
            if (nameMatch && nameMatch[1].toLowerCase() === 'action') {
                 params.set('action', 'default');
             } else if (nameMatch && !params.has(nameMatch[1])) {
                 params.append(nameMatch[1], valueMatch ? valueMatch[1] : '');
             }
        }
    }
    // Set username instead of appending, or wait, CareLink might use j_username
    console.log(params.toString().substring(0, 100));
}
