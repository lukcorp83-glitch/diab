...
for (const sourceFile of project.getSourceFiles()) {
    ...
    sourceFile.forEachDescendant((node) => {
        ...
        if (Node.isStringLiteral(node) && node.getParentIfKind(SyntaxKind.PropertyAssignment)) {
            ...
            if (polishRegex.test(text)) {
                ...
                const allowedProps = ["label", "name", "desc", "sub", "title", "content", "question", "answer", "tooltip", "category", "cons", "a", "q", "tips", "description", "pros", "text", "message", "unit", "placeholder"];
        ...
        if (Node.isStringLiteral(node) && node.getParentIfKind(SyntaxKind.ArrayLiteralExpression)) {
            const text = node.getLiteralValue();
            if (polishRegex.test(text)) {
                // To avoid breaking random arrays of IDs, we only translate strings with spaces or Polish chars explicitly.
                const key = `auto.${createKey(text)}`;
                translations[key] = text;
                replacements.push({ node, text: `i18n.t('${key}', { defaultValue: ${JSON.stringify(text)} })` });
            }
    ...
    if (replacements.length > 0) {
        ...
        if (!hasI18nImport) {
            const isApp = sourceFile.getFilePath().endsWith("App.tsx") || sourceFile.getFilePath().endsWith("constants.ts");
